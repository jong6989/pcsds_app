myAppModule.
controller('CertificationController', function($scope){
    const time_key = 'created_time';
    const collection = 'documents';
    $scope.certification_forms = [
        { 
            subject: 'No Pending Case',
            category: 'no_pending_case_certification',
            template: {
                ...
                'list_view_template': './app/templates/templates/certificate_npc/listview.html'
            }
        },
        {
            subject: 'Certificate of Inspection (LTP)',
            category: 'ltp_inspection_certification',
            template: {
                ...
                'list_view_template': './app/templates/templates/ltp_wildlife/certificate/listview.html'
                
            }
        }
    ];
    $scope.n = $scope.certification_forms[0];

    var viewPath = "/certification/view";
    var listPath = "/certification/list/all";

    $scope.setSelectedIndex = (index) =>{
        $scope.n = $scope.certification_forms[index];
    }
    ...
    $scope.load_data = () => {
        if($scope.url.has('ID')){
            ...
            if($scope.currentItem == undefined) $scope.set_path('/certification/create');  
        }
    };
    ...
    $scope.getSelectedCertification = (category) => {
        var index = $scope.certification_forms.findIndex(item => item.category == category);
        return $scope.certification_forms[index];
    }
