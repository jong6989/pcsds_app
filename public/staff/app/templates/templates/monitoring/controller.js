myAppModule.
    ...
    controller('SEPMonitoringController', function ($scope, galleryService) {
        $scope.n.compliances = [
        ...
        $scope.addToGallery = galleryService.addToGallery;

        $scope.removeFromGallery = galleryService.removeFromGallery;
    }).
    controller('GravelAndSandMonitoringController', function ($scope, galleryService) {
        $scope.n.compliances = [
            { terms: "", uploadedImages: [] },
            { terms: "In case of over-extraction outside the approved area as may be determined by the ECAN Board of the PCSDS, the same shall serve as valid grounds for the automatic cancellation of this clearance.", uploadedImages: [] },
            { terms: "The depth of the excavation should not exceed 1 meter depth.", uploadedImages: [] },
            {
                terms: "The herein grantee shall assume full responsibility and liability for" +
                    " damages to private/public property caused by the project."
            },
            { terms: "Install visible markings to mark the boundary of the applied area prior to quarry operations.", uploadedImages: [] },
            {
                terms: "Limit quarrying of gravel and sand deposits to mid- section of the river and established buffer zone of at least five (5) meters from each side of the riverbanks must always be observed.", uploadedImages: []
            },
            { terms: "", uploadedImages: [] },
            {
                terms: "Extraction of gravel and sand within the applied area must commence downstream and to progress upstream.", uploadedImages: []
            },
            { terms: "The proponent should institute rehabilitation measures by backfilling the riverbanks with boulders after which endemic trees and grass species be planted along the backfilled riverbanks during quarry operations.", uploadedImages: [] },
            {
                terms: "The SEP Clearance shall be required upon the 4th renewal of such permit. Provided further, that in the annual renewal thereof, the permitting agency shall require the proponent to submit a certification from the PCSDS stating that the subject proponent has complied with all the terms and conditions of the SEP clearance and has no pending case administrative case before the PCSD Adjudication Board. Provided finally, that the PCSD may impose additional requirements on the operation to safeguard the integrity of the environment based on new findings from technical studies on the quarry sites.", uploadedImages: []
            },
            { terms: "The proponent shall support governments projects by making aggregates available.", uploadedImages: [] }
        ];
        $scope.n.compliances[0].terms = "Confine the quarry operations within the allowed area of " + 
            $scope.n.compliances[0].allowed_area + "has.";
        $scope.n.compliances[5].terms = "Limit extraction to applied volume of " + 
            $scope.n.compliances[5].volume_limitation + 
            " cubic meters per annum. However, in the event that the applied volume is projected to be consumed before the expiration of  the permit the proponent must file an application for additional volume 60 days before the projected consummation of the volume stipulated in the permit.";
        $scope.addToGallery = galleryService.addToGallery;
        $scope.removeFromGallery = galleryService.removeFromGallery;
    }).
    service('galleryService', function(Upload){
        var imageID = 1;
        this.addToGallery = (files, imageCollection, index) => {
            Upload.
                base64DataUrl(files).
                then(urls => {
                    urls.forEach(url => {
        ...
        this.removeFromGallery = (imageToRemove, onDelete) => {
