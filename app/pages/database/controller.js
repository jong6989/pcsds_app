...
myAppModule.controller('pcsd_database_controller', function ($scope, $timeout, $utils, $mdToast,$localStorage) {
    ...
    async function loadFireDb(k) {
        let a = await fire.db.database.query.doc(k).collection("database").get();
        let res = a.docs.map( doc => {
            let b = doc.data();
            b.id = doc.id;
            return b;
        });
    ...
    if($scope.wsup_db.data.length == 0){
        console.log('Loading...');
        fire.db.database.get("WSUP",(d) => {
            console.log(d);
            $scope.wsup_db.summary = $localStorage.wsup_db_summary = d;
        });
        loadFireDb("WSUP").then( res => {
            console.log(res);
